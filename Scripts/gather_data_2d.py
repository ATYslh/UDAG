...
def mask_data(input_file: str, mask2d, variable: str):
    ...
    fldmean = ds[variable].where(mask2d == 0)
...
def create_info_json(output_folder):
    ...
    write_json_file(
        f"/work/bb1364/g260190_heinrich/UDAG/Data_2d/json_files/{project}_{country}_{variable}_info.json",
...
def main():
    ...
    for variable in variables:
        output_folder = (
            f"/work/bb1364/g260190_heinrich/UDAG/Data_2d/{project}/{country}/{variable}"
        )
        if not os.path.exists(output_folder):
            os.mkdir(output_folder)
        ...
        for spatial_resolution in ["EUR-12", "MEU-3", "CEU-3"]:
            ...
            for temporal_resolution in list_of_wanted_resolutions:
                ...
                for input_folder in data_folders:
                    if "evaluation" not in input_folder:
                        continue
